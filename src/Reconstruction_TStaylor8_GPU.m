function [ori_A_adj, P3_tensor] = Reconstruction_TStaylor8_GPU(UAU_state_nodes, SIS_state_nodes)
  ...
  parfor nod = 1:n
      try
          ...
          % ------------------- 求解 ------------------- 
          [A2,neig_subset] = solveTS1(X,Y,theta1,A2,nod);
    
          %% step two --- Taylor2
          [Oi, Vi, Wi, A3] = solveTS2(A2, X, Y);
          
          P0 = lsqnonlin_Opt(A2,A3,Oi,Vi,Wi,X,Y);
...
%% 计算残差和雅可比矩阵（直接计算版本）
function [R,J] = compute_residual_torch(theta,A2,A3,Oi,Vi,Wi,X,Y)
    % 初始化\alpha
...
function [A2,neig_subset] = solveTS1(X,Y,theta1,A2, nod)
    % 初始化\alpha
    lambda = 0.1;       alpha = gpuArray(log(1-lambda));
...
function [Oi, Vi, Wi, A3] = solveTS2(A2, X, Y)
    theta1=sum(A2,2);  % \={theta}^i(t_m), in Eq(4.22)
...
function P0 = lsqnonlin_Opt(A2,A3,Oi,Vi,Wi,X,Y)
% ------------------- 最小二乘 -------------------
    fun = @(x) compute_residual_torch(x,A2,A3,Oi,Vi,Wi,X,Y);  % 注意参数传递，尤其是x
